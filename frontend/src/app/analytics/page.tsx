...
export default function AnalyticsPage() {
  ...
  const failedGradings = gradings.filter(g => g.status === 'failed');
  ...
  return (
    <main className="app-container">
      <header className="app-header">
        ...
        <p>View all your presentation gradings and performance insights</p>
      </header>
      ...
      <div className="analytics-content">
          ...
          <div className="section-header-with-pagination">
            ...
            {gradings.length > 0 && (
              <div className="pagination-info">
                Showing {Math.min((currentPage - 1) * itemsPerPage + 1, gradings.length)}-{Math.min(currentPage * itemsPerPage, gradings.length)} of {gradings.length}
              </div>
            )}
          </div>
        </div>
        ...
        {gradings.length === 0 ? (
          <div className="empty-state">
            ...
            <h2>No Gradings Yet</h2>
            <p>Grade your first presentation to see analytics here</p>
          ...
          <>
            <div className="gradings-grid">
              {gradings.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage).map(grading => (
                <div
                  key={grading.id}
            ...
            {gradings.length > itemsPerPage && (
              <div className="pagination-controls">
                ...
                <div className="pagination-pages">
                  {Array.from({ length: Math.ceil(gradings.length / itemsPerPage) }, (_, i) => i + 1).map(page => (
                    <button
                      key={page}
                ...
                <button
                  className="pagination-btn"
                  onClick={() => setCurrentPage(p => Math.min(Math.ceil(gradings.length / itemsPerPage), p + 1))}
                  disabled={currentPage === Math.ceil(gradings.length / itemsPerPage)}
                >
                  Next
                  <svg fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}>
